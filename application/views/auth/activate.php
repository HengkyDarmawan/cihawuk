		...
		<div class="card-soft auth-card">
			<div class="auth-form-inner">
				...
				<?php if ( ! empty($token_invalid)): ?>
					<div class="alert alert-warning d-flex gap-2 auth-inline-error" role="alert" data-swal-alert="warning"><?= icon('alert-triangle') ?><div>Kode aktivasi tidak valid, sudah digunakan, atau kedaluwarsa. Minta kode baru di kantor desa.</div></div>
				<?php endif; ?>
				<?php if ( ! empty($activate_error)): ?>
					<div class="alert alert-danger d-flex gap-2 auth-inline-error" role="alert" data-swal-alert="error"><?= icon('alert-circle') ?><div><?= e($activate_error) ?></div></div>
				<?php endif; ?>
