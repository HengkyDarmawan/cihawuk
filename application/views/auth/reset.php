...
<section class="auth-shell">
	<div class="card-soft auth-card">
		...
		<?php if ( ! empty($token_invalid)): ?>
			<div class="alert alert-warning d-flex gap-2" role="alert"><?= icon('alert-triangle') ?><div>Tautan reset tidak valid, sudah dipakai, atau kedaluwarsa.</div></div>
		<?php endif; ?>
		<?php if ( ! empty($reset_error)): ?>
			<div class="alert alert-danger d-flex gap-2" role="alert"><?= icon('alert-circle') ?><div><?= e($reset_error) ?></div></div>
		<?php endif; ?>
