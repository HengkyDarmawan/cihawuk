...
<div class="row">
	<div class="col-lg-5 mb-4">
		<form class="card shadow-sm" method="post" action="<?= site_url('admin/ekspor') ?>" data-once>
			<div class="card-header"><h2>Minta rekap baru</h2></div>
			<div class="card-body">
				<?= csrf_field() ?>
				<?= ui_select(array('name' => 'report_type', 'label' => 'Jenis rekap', 'required' => TRUE, 'options' => $report_types, 'value' => 'tickets')) ?>
				<?= ui_select(array('name' => 'format', 'label' => 'Format', 'required' => TRUE, 'options' => array('csv' => 'CSV (baris data)', 'pdf' => 'PDF (ringkasan)'), 'value' => 'csv')) ?>
				<?= ui_select(array('name' => 'status', 'label' => 'Status', 'options' => array_map(function ($s) { return $s['staff']; }, $statuses), 'placeholder_option' => 'Semua status', 'value' => '')) ?>
				<?= ui_select(array('name' => 'jenis', 'label' => 'Jenis laporan', 'options' => $report_kinds, 'placeholder_option' => 'Semua jenis', 'value' => '')) ?>
				<div class="form-row">
			...
			<div class="card-footer bg-white text-right">
				<button class="btn btn-primary" type="submit">Minta ekspor</button>
			</div>
		</form>
	</div>
	...
	<div class="col-lg-7 mb-4">
		<div class="card shadow-sm">
			...
			<div class="card-body">
				<?php if (empty($jobs)): ?>
					<div class="empty-box"><i class="fas fa-file-export" aria-hidden="true"></i><p class="mb-0">Belum ada permintaan ekspor.</p></div>
				<?php else: ?>
